...
/**
 * Ported from
 * <a href="http://developer.nvidia.com/object/nvtristrip_library.html">NVIDIA's
...
public final class NvStripifier {
	 ...
	 */
	// Equinox-: Made Public
	public void stripify(final List<Integer> in_indices, final int in_cacheSize, final int in_minStripLength,
		...
		// this is the strip size threshold below which we dump the strip into a
		// list
	 ...
	 */
	// Equinox-: Made Public
	public int createStrips(final List<NvStripInfo> allStrips, final List<Integer> stripIndices,
			final boolean bStitchStrips, final boolean bRestart, final int restartVal) {
	...
	static NvEdgeInfo findEdgeInfo(final List<NvEdgeInfo> edgeInfos, final int v0, final int v1) {
		// we can get to it through either array because the edge infos have a
		// v0 and v1 and there is no order except
	...
	/**
	 * A good reset point is one near other committed areas so that we know that
	 * when we've made the longest strips its because we're stripifying in the
	...
	NvFaceInfo findGoodResetPoint(final List<NvFaceInfo> faceInfos, final List<NvEdgeInfo> edgeInfos) {
		// we hop into different areas of the mesh to try to get other large
		// open spans done. Areas of small strips can
	 ...
	 * 
	 * Works by setting running a number of experiments in different areas of
	 * the mesh, and accepting the one which results in the longest strips. It
	 * then accepts this, and moves on to a different area of the mesh. We try
	 * to jump around the mesh some, to ensure that large open spans of strips
	 * get generated.
	 * 
	 * @param allStrips
	 * @param allFaceInfos
	...
	void findAllStrips(final List<NvStripInfo> allStrips, final List<NvFaceInfo> allFaceInfos,
		...
		while (!done) {
			...
			// and really build each of the strips and strips that follow to see
			// how
			// far we get
			//
	 ...
	 * friendly pieces, then reorders these pieces to maximize cache hits. The
	 * final strips are stored in outStrips
	 * 
	 * @param allStrips
	 * @param outStrips
	 * @param edgeInfos
	...
	void splitUpStripsAndOptimize(final List<NvStripInfo> allStrips, final List<NvStripInfo> outStrips,
		...
		for (int i = 0; i < allStrips.size(); i++) {
			...
			if (actualStripSize > threshold) {
				...
				// we're not just doing a tempStrips.add(allBigStrips.get(i))
				// because
		...
		if (tempStrips2.size() != 0) {
			...
			// XXX: this n^2 algo is what slows down stripification so much....
			...
			while (true) {
				...
				for (int i = 0; i < tempStrips2.size(); i++) {
					...
					if (numHits > bestNumHits) {
						...
						// If there is a second face, reorder vertices such that
						// the
						// unique vertex is first
						if (nStripFaceCount > 1) {
							final int nUnique = NvStripifier.getUniqueVertexInB(strip._faces.get(1), tFirstFace);
	...
	/**
	 * @param allStrips
	 *            the whole strip vector...all small strips will be deleted from
	 *            this list, to avoid leaking mem
	 * @param allBigStrips
	 *            an out parameter which will contain all strips above
	 *            minStripLength
	 * @param faceList
	 *            an out parameter which will contain all faces which were
	...
	void buildStripifyInfo(final List<NvFaceInfo> faceInfos, final List<NvEdgeInfo> edgeInfos, final int maxIndex) {
		...
		for (int i = 0; i < numTriangles; i++) {
			...
			if (edgeInfo01 == null) {
				...
				// can't already exist in the face
				// structure
